...
| allowFloorAttach | Bool | `true` | Whether or not to enable attaching to floors either through default behaviour or when explicitly set. If disabled, will remain in an airborne state. |
| allowWallAttach | Bool | `true` | Whether or not to enable attaching to walls either through default behaviour (i.e. wall grazing) or when explicitly set. |
